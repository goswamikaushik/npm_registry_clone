...
export default function HomePage() {
  ...
  const renderedPackages = featuredPackages.map((p) => {
    return (
      <div
        key={p.name}
        className="flex flex-col justify-between gap-3 border border-blue-300 rounded shadow p-4"
      >
        ...
        <Link
          to={`/packages/${p.name}`}
          className="border border-blue-500 rounded border-gray-900 text-center"
        >
          View
        </Link>
      </div>
  ...
  return (
    <div className="container py-12 space-y-8">
      <div className="space-y-6 text-center">
        ...
        <p className="mx-auto max-w-[600px] text-red-700">
